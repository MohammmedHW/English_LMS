...
@section('title', 'Edit Test Question')
...
<div class="d-flex align-items-center justify-content-between mb-4">
    <div>
        <h1 class="h4 mb-1">Edit Test Question</h1>
        <p class="text-muted small mb-0">
            <i class="fas fa-edit me-1 opacity-50"></i>
            {{ $testQuestion->test->lesson->title }}
            <span class="mx-2 opacity-25">|</span>
            Order #{{ $testQuestion->order }}
        </p>
    </div>
    <a href="{{ route('lessons.test_questions.index', $testQuestion->test->lesson_id) }}" class="btn btn-sm btn-light border">
...
<form action="{{ route('test_questions.update', $testQuestion) }}" method="POST" id="edit-form">
    @csrf
    @method('PUT')
    ...
    <div class="question-card card border-0 shadow-sm mb-5">
        <div class="card-header bg-white py-3 px-4">
            <div class="d-flex align-items-center gap-3">
                <span class="question-index-badge">TEST</span>
        ...
        <div class="card-body px-4 pt-3 pb-4">
            ...
            <div class="mb-4">
                ...
                <select name="type" class="form-select form-select-sm type-select" id="type-select">
                    @foreach(['Multiple Choice', 'True/False', 'Fill in the Blank', 'Complete the Sentence', 'Match the Pair'] as $type)
                        <option value="{{ $type }}" {{ $testQuestion->question->type === $type ? 'selected' : '' }}>{{ $type }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="question" class="form-control" rows="3" required placeholder="Enter the question here...">{{ $testQuestion->question->question }}</textarea>
            ...
            <div class="type-section mcq-section d-none">
                    ...
                    <div class="col-md-6">
                        <div class="input-group input-group-sm">
                            ...
                            <input type="text" name="options[]" class="form-control" placeholder="Option {{ chr(65 + $i) }} {{ $i > 1 ? '(optional)' : '' }}" 
                                value="{{ isset($testQuestion->question->options[$i]) ? $testQuestion->question->options[$i] : '' }}">
                        </div>
                    </div>
                    @endfor
                </div>
                ...
                <input type="text" name="answer_mcq" class="form-control form-control-sm correct-input"
                    value="{{ $testQuestion->question->type === 'Multiple Choice' ? $testQuestion->question->answer : '' }}"
            ...
            <div class="type-section tf-section d-none">
                ...
                <div class="d-flex gap-3">
                    <label class="tf-option">
                        <input type="radio" name="answer_tf" value="True" {{ $testQuestion->question->answer === 'True' ? 'checked' : '' }}>
                        <span class="tf-label true-label"><i class="fas fa-check me-2"></i>True</span>
                    </label>
                    <label class="tf-option">
                        <input type="radio" name="answer_tf" value="False" {{ $testQuestion->question->answer === 'False' ? 'checked' : '' }}>
            ...
            <div class="type-section fib-section d-none">
                ...
                <input type="text" name="answer_fib" class="form-control form-control-sm correct-input"
                    value="{{ $testQuestion->question->type === 'Fill in the Blank' ? $testQuestion->question->answer : '' }}"
            ...
            <div class="type-section cts-section d-none">
                ...
                <input type="text" name="answer_cts" class="form-control form-control-sm correct-input"
                    value="{{ $testQuestion->question->type === 'Complete the Sentence' ? $testQuestion->question->answer : '' }}"
            ...
            <div class="type-section mtp-section d-none">
                ...
                <div class="pair-container mb-2">
                    @php
                        $pairs = $testQuestion->question->type === 'Match the Pair' ? $testQuestion->question->options : [['left' => '', 'right' => '']];
            ...
            <input type="hidden" name="answer" id="final-answer" value="{{ $testQuestion->question->answer }}">
    ...
    <div class="d-flex justify-content-end gap-2 mb-5">
        <a href="{{ route('lessons.test_questions.index', $testQuestion->test->lesson_id) }}" class="btn btn-light px-4 border">Cancel</a>
...
@section('styles')
<style>
    .question-card { border-radius: 0.75rem; border-left: 3px solid #f59e0b !important; }
    .question-index-badge { display: inline-flex; align-items: center; justify-content: center; width: 45px; height: 30px; border-radius: 20px; background: #fffbeb; color: #d97706; font-size: 0.65rem; font-weight: 800; }
