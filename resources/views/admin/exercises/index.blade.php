...
@section('title', 'Practice Exercises — ' . $lesson->title)
...
<div class="d-flex align-items-center justify-content-between mb-4">
    <div>
        <h1 class="h4 mb-1">Practice Exercises</h1>
        <p class="text-muted small mb-0">
            <i class="fas fa-circle-play me-1 opacity-50"></i>
            {{ $lesson->title }}
            <span class="mx-2 opacity-25">|</span>
            {{ $exercises->count() }} exercise{{ $exercises->count() !== 1 ? 's' : '' }} total
        </p>
    </div>
    <div class="d-flex gap-2">
        ...
        <a href="{{ route('lessons.exercises.create', $lesson) }}" class="btn btn-sm btn-primary px-3 shadow-none">
            ...
            <table class="table table-hover align-middle mb-0 custom-table">
                <thead>
                    <tr>
                        ...
                        <th class="px-4 py-3 text-muted fw-bold small text-uppercase text-end">Action</th>
                    </tr>
                </thead>
                <tbody class="border-top-0">
                    ...
                    <tr class="fade-in">
                        ...
                        <td class="px-4 py-3">
                            <span class="type-badge type-{{ Str::slug($exercise->question->type) }}">
                                {{ $exercise->question->type }}
                            </span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="question-preview" title="{{ $exercise->question->question }}">
                                {{ Str::limit($exercise->question->question, 60) }}
                            </div>
                        </td>
                        <td class="px-4 py-3">
                            <code class="text-success small fw-bold">
                                {{ Str::limit($exercise->question->answer, 30) }}
                            </code>
                        </td>
                        <td class="px-4 py-3 text-end">
                            <div class="btn-group">
                                <a href="{{ route('exercises.edit', $exercise) }}" class="btn btn-action" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="{{ route('exercises.destroy', $exercise) }}" method="POST" class="d-inline ms-1">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="text-center py-5">
                            <div class="empty-state py-4">
                                ...
                                <h6 class="text-muted mb-1">No Exercises Found</h6>
                                <p class="small text-muted mb-3">Add practice exercises to help students learn better.</p>
                                <a href="{{ route('lessons.exercises.create', $lesson) }}" class="btn btn-sm btn-primary">
...
<style>
    ...
    .order-badge { font-weight: 700; color: #64748b; font-size: 0.85rem; }
