...
@section('title', 'Add Exercises — ' . $lesson->title)
...
<div class="d-flex align-items-center justify-content-between mb-4">
    <div>
        <h1 class="h4 mb-1">Add Practice Exercises</h1>
        <p class="text-muted small mb-0">
            <i class="fas fa-circle-play me-1 opacity-50"></i>
            {{ $lesson->title }}
            <span class="mx-2 opacity-25">|</span>
            <a href="{{ route('lessons.exercises.index', $lesson) }}" class="text-decoration-none text-muted">View all exercises</a>
        </p>
    </div>
    <a href="{{ route('lessons.exercises.index', $lesson) }}" class="btn btn-sm btn-light border">
...
<form action="{{ route('lessons.exercises.store', $lesson) }}" method="POST" id="bulk-form">
    @csrf
    ...
    <div class="d-flex align-items-center gap-2 mt-2 mb-5 p-3 bg-white border rounded-3 shadow-sm">
        ...
        <div class="ms-auto d-flex gap-2">
            <a href="{{ route('lessons.exercises.index', $lesson) }}" class="btn btn-sm btn-light border">Cancel</a>
            <button type="submit" class="btn btn-sm btn-primary px-4" id="save-all-btn">
                <i class="fas fa-check me-2"></i>Save Exercises
            </button>
        </div>
    </div>
</form>

{{-- Question Card Template (NOT inside <form>, cloned via JS into form) --}}
    ...
    <div class="question-card card border-0 shadow-sm mb-4">
        ...
        <div class="card-body px-4 pt-3 pb-4">
            ...
            <div class="type-section fib-section d-none" data-section="Fill in the Blank">
                ...
                <input type="text" name="questions[__IDX__][answer]" class="form-control form-control-sm correct-input"
                    placeholder="The word that fills the blank (e.g. goes)">
            </div>
            ...
            <div class="type-section mtp-section d-none" data-section="Match the Pair">
                ...
                <div class="pair-container mb-2">
                    <div class="pair-row row g-2 align-items-center mb-2">
                        <div class="col-md-5">
                            <input type="text" name="questions[__IDX__][pairs][left][]" class="form-control form-control-sm" placeholder="Left item (e.g. Cat)">
                        </div>
                        <div class="col-auto text-muted"><i class="fas fa-arrow-right-long"></i></div>
                        <div class="col-md-5">
                            <input type="text" name="questions[__IDX__][pairs][right][]" class="form-control form-control-sm" placeholder="Right item (e.g. Animal)">
                        </div>
                        <div class="col-auto">
                ...
                {{-- Hidden answer field for MTP type --}}
...
@section('styles')
<style>
    .question-card { border-radius: 0.75rem; border-left: 3px solid #4f46e5 !important; transition: box-shadow 0.2s; }
...
const TYPE_CONFIG = {
    ...
    'Fill in the Blank':     { section: 'fib-section', label: 'Sentence with Blank',     hint: 'Use ___ to mark the blank. E.g. "She ___ to school every day."' },
...
/** Disable all inputs in hidden sections, enable inputs in active section */
...
function addQuestion() {
    ...
    const idx      = questionCount; // use current count as index
    questionCount++;

    // Replace __IDX__ placeholder throughout
    const html = new XMLSerializer().serializeToString(clone);
    // We'll do it via iterating nodes instead
    function replaceIdx(node) {
        if (node.nodeType === Node.TEXT_NODE) {
            node.textContent = node.textContent.replaceAll('__IDX__', idx);
        } else if (node.nodeType === Node.ELEMENT_NODE) {
            ['name', 'id', 'for', 'data-section'].forEach(attr => {
                if (node.hasAttribute(attr)) {
                    node.setAttribute(attr, node.getAttribute(attr).replaceAll('__IDX__', idx));
                }
            });
            node.childNodes.forEach(replaceIdx);
        }
    }
    clone.childNodes.forEach(replaceIdx);
    ...
    card.querySelector('.add-pair-btn').addEventListener('click', function () {
        ...
        newRow.querySelector('.remove-pair-btn').addEventListener('click', () => removePairRow(newRow));
        container.appendChild(newRow);
    });

    // Wire up existing remove-pair buttons
    card.querySelectorAll('.remove-pair-btn').forEach(btn => {
        btn.addEventListener('click', function () { removePairRow(this.closest('.pair-row')); });
    });

    // Initialize active type (MCQ by default) — disable all hidden sections
...
function updateUI() {
    ...
    document.getElementById('save-all-btn').innerHTML  =
        `<i class="fas fa-check me-2"></i>Save ${count > 0 ? count + ' Exercise' + (count !== 1 ? 's' : '') : 'Exercises'}`;
}

// Before submit: ensure all hidden sections are disabled
document.getElementById('bulk-form').addEventListener('submit', function (e) {
    ...
    if (document.querySelectorAll('.question-card').length === 0) {
        e.preventDefault();
        alert('Please add at least one exercise question before saving.');
    }
});

document.addEventListener('DOMContentLoaded', () => {
    addQuestion(); // Start with one blank card
